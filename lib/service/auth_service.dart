...
class AuthService_Rizwar { 
  ...
  String? validateEmail_Rizwar(String? value) { 
    ...
    const String campusDomain_R = r'@kampus\.ac\.id$'; 
    final RegExp emailRegex_R = RegExp(r'^[a-zA-Z0-9.]+'+ campusDomain_R);

    if (!emailRegex_R.hasMatch(value)) {
      return 'Wajib gunakan format email yang valid dan domain kampus (ex: @kampus.ac.id)!';
    }
    return null;
  }
  ...
  String? validatePassword_R(String? value) { 
    if (value == null || value.length <= 6) { // Memastikan length > 6
  ...
  Future<User?> signInUser_R(String email, String password) async { 
    // Tangani error di UI
    String userCredential_R = await _auth_R.signInWithEmailAndPassword(
      email: email,
      password: password,
    );
  ...
  Future<User?> registerUser_R({ 
    ...
    // 1. Buat Akun Auth
    String userCredential_R = await _auth_R.createUserWithEmailAndPassword(
      email: email,
      password: password,
    );
